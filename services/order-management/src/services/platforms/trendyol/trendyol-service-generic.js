...
class TrendyolServiceGeneric {
  ...
  async fetchOrders(params = {}) {
    try {
      ...
      const response = await this.apiClient.get('/suppliers/orders', { params: queryParams });
  ...
  async fetchProducts(params = {}) {
    try {
      ...
      const response = await this.apiClient.get('/suppliers/products', { params: queryParams });
  ...
  async syncOrders(params = {}) {
    try {
      ...
      const { Order, OrderItem, ShippingDetail, PlatformData, PlatformAttribute, OrderHistory } = models;
      ...
      for (const trendyolOrder of orders) {
        try {
          ...
          logger.error(`Error processing Trendyol order: ${orderError.message}`, {
            error: orderError,
  ...
  async updateExistingOrder(existingOrder, trendyolOrderData) {
    ...
    try {
      ...
      logger.debug(`Updated order ${existingOrder.externalOrderId} from Trendyol`);
  ...
  async createNewOrder(trendyolOrderData) {
    const { Order, OrderItem, ShippingDetail, PlatformData, PlatformAttribute } = require('../../../models');
    const transaction = await sequelize.transaction();
    ...
    try {
      ...
      if (trendyolOrderData.shipmentAddress) {
        await ShippingDetail.create({
          ...
          phone: trendyolOrderData.shipmentAddress.phone || orderData.customerPhone,
  ...
  mapTrendyolOrderToInternal(trendyolOrder) {
    ...
    const customerName = trendyolOrder.customerFirstName && trendyolOrder.customerLastName 
    ...
    return {
      ...
      paymentStatus: this.mapPaymentStatus(trendyolOrder.status),
  ...
  mapPaymentStatus(trendyolStatus) {
    ...
    const cancelledStatuses = ['Cancelled', 'Returned', 'UndeliveredAndReturned'];
  ...
  async syncProducts(params = {}) {
    try {
      const { Product, PlatformData, PlatformAttribute } = require('../../../models');
      ...
      for (const trendyolProduct of products) {
        try {
          ...
          if (existingPlatformData) {
            ...
            await this.updateExistingProduct(existingPlatformData, trendyolProduct);
            stats.updated++;
          } else {
  ...
  async updateExistingProduct(existingPlatformData, trendyolProductData) {
    const { Product, PlatformData, PlatformAttribute, ProductPriceHistory } = require('../../../models');
    const transaction = await sequelize.transaction();
    ...
    try {
      ...
      if (mainProduct) {
        ...
        const newPrice = parseFloat(trendyolProductData.listPrice) || currentPrice;
      ...
      logger.debug(`Updated product ${existingPlatformData.platformEntityId} from Trendyol`);
  ...
  async createNewProduct(trendyolProductData, userId) {
    ...
    try {
      ...
      // Create platform data record
      const externalProductId = trendyolProductData.productCode || trendyolProductData.id.toString();
      ...
      logger.debug(`Created new product ${platformData.platformEntityId} from Trendyol`);
