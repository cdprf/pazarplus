...
class ProductMergeService {
  ...
  async mergeProducts(products) {
    ...
    products.forEach((product) => {
      ...
      const sku = this.extractSku(product);
      if (sku && productGroups[sku].length > 1) {
        return;
      }
  ...
  async processSingleProduct(mergedProduct, userId, transaction) {
    ...
    if (existingProduct) {
      ...
      const productData = {
        ...
        creationSource: mergedProduct.sourcePlatform ? "platform_sync" : "user", // Set based on source
  ...
  calculateProductCompleteness(product) {
    let score = 0;
    if (this.extractName(product)) {
      score += 10;
    }
    if (this.extractSku(product)) {
      score += 10;
    }
    if (this.extractBarcode(product)) {
      score += 8;
    }
    if (this.extractDescription(product)) {
      score += 6;
    }
    if (this.extractPrice(product) > 0) {
      score += 8;
    }
    if (this.extractStockQuantity(product) > 0) {
      score += 8;
    }
    if (this.extractCategory(product)) {
      score += 6;
    }
    const images = this.extractImages(product);
  ...
  extractVariantAttributes(product) {
    ...
    if (product.pattern) {
      attributes.pattern = product.pattern;
    }
    if (product.rawData) {
      if (product.rawData.color) {
        attributes.color = product.rawData.color;
      }
      if (product.rawData.size) {
        attributes.size = product.rawData.size;
      }
      if (product.rawData.material) {
        attributes.material = product.rawData.material;
      }
      if (product.rawData.pattern) {
        attributes.pattern = product.rawData.pattern;
      }
  ...
  extractDimensions(product) {
    ...
    if (product.depth) {
      dimensions.depth = parseFloat(product.depth);
    }
    if (product.rawData) {
  ...
  extractErrorInfo(product) {
    const errorInfo = {};
    if (product.error) {
      errorInfo.error = product.error;
    }
    if (product.errorMessage) {
      errorInfo.errorMessage = product.errorMessage;
    }
    if (product.errorCode) {
      errorInfo.errorCode = product.errorCode;
    }
    if (product.hasError) {
      errorInfo.hasError = product.hasError;
    }
    if (product.rawData) {
      if (product.rawData.error) {
        errorInfo.error = product.rawData.error;
      }
  ...
  extractUrls(product) {
    ...
    if (product.productUrl) {
      urls.productUrl = product.productUrl;
    }
    if (product.platformUrl) {
      urls.platformUrl = product.platformUrl;
    }
    if (product.detailUrl) {
      urls.detailUrl = product.detailUrl;
    }
    if (product.rawData) {
      ...
      if (product.rawData.detailUrl) {
        urls.detailUrl = product.rawData.detailUrl;
      }
    }
  ...
  derivePlatformFromConnectionName(connectionName) {
    ...
    if (name.includes("trendyol") || name.includes("ty")) {
      return "trendyol";
    }
    if (name.includes("n11")) {
      return "n11";
    }
    if (name.includes("amazon")) {
      return "amazon";
    }
    if (name.includes("gittigidiyor")) {
      return "gittigidiyor";
    }
