...
class ExportService {
  ...
  async exportOrdersToCSV(filters = {}, userId) {
    try {
      ...
      const flattenedOrders = orders.map(order => this.flattenOrderData(order));
      ...
      logger.error(`Failed to export orders to CSV: ${error.message}`, { error });
  ...
  async exportOrdersToExcel(filters = {}, userId) {
    try {
      ...
      logger.error(`Failed to export orders to Excel: ${error.message}`, { error });
  ...
  async exportOrderItemsToCSV(filters = {}, userId) {
    try {
        ...
        if (order.OrderItems && order.OrderItems.length > 0) {
          order.OrderItems.forEach(item => {
            orderItems.push({
              ...
              variantInfo: item.variantInfo ? JSON.stringify(item.variantInfo) : '',
      ...
      logger.error(`Failed to export order items to CSV: ${error.message}`, { error });
