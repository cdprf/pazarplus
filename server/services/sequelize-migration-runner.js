...
class SequelizeMigrationRunner {
  ...
  async tableExists(tableName) {
    try {
      ...
      this.logger.warn(`Error checking if table ${tableName} exists:`, error.message);
      return false;
    }
  }
  ...
  async getExecutedMigrations() {
    try {
      ...
      if (this.sequelize.getDialect() === 'sqlite' && 
        ...
        this.logger.info('Using in-memory database, no migration tracking available');
        return [];
      }
  ...
  async markMigrationExecuted(migrationName) {
    try {
      ...
      if (error.name === 'SequelizeUniqueConstraintError') {
        this.logger.debug(`Migration ${migrationName} already marked as executed`);
        return;
      }
      this.logger.warn(`Error marking migration ${migrationName} as executed:`, error.message);
    }
  }
  ...
  async executeMigration(migrationFile) {
    ...
    try {
      ...
      if (error.name === 'SequelizeDatabaseError' && 
          (error.original?.code === '42P07' || error.message.includes('already exists'))) {
        this.logger.info(`⏭️  Migration skipped (table already exists): ${migrationFile}`);
  ...
  async runMigrations() {
    try {
      ...
      this.logger.info(`🎉 Database migrations completed! ${successCount}/${pendingMigrations.length} migrations executed successfully.`);
      return true;
  ...
  async runComprehensiveMigration() {
    const comprehensiveMigrationFile = '20250621000001-create-all-tables-complete.js';
    const migrationFiles = await this.getMigrationFiles();
    ...
    if (migrationFiles.includes(comprehensiveMigrationFile)) {
      ...
      if (!executedMigrations.includes(comprehensiveMigrationFile)) {
        this.logger.info(`Running comprehensive migration: ${comprehensiveMigrationFile}`);
        await this.executeMigration(comprehensiveMigrationFile);
        return true;
      } else {
        this.logger.info(`Comprehensive migration already executed: ${comprehensiveMigrationFile}`);
        return true;
      }
    }
