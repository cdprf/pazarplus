  ...
  async getPlatformService(platform, userId, connectionId) {
    try {
      ...
      if (!actualConnectionId) {
        ...
        this.logger.info(`Found connection for ${platform}`, {
          operation: "platform_connection",
          platform,
          userId,
    ...
    try {
      ...
      for (const platformCategory of platformCategories) {
        try {
          ...
          if (created) {
            this.logger.info(`Created new category: ${categoryData.name}`);
          } else {
            this.logger.info(`Updated existing category: ${categoryData.name}`);
          }
        } catch (categoryError) {
          this.logger.error(
  ...
  applyFieldMappings(categoryData, fieldMappings) {
    ...
    for (const [sourceField, targetField] of Object.entries(fieldMappings)) {
      if (categoryData[sourceField] !== undefined && targetField) {
        ...
        this.logger.info(`🔄 Mapped field: ${sourceField} -> ${targetField}`);
      }
    }
