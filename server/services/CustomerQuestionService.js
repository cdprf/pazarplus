...
class CustomerQuestionService {
  ...
  async getQuestions(options = {}) {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        whereClause.creation_date[Op.gte] = startDate;
      }
      if (endDate) {
        whereClause.creation_date[Op.lte] = endDate;
      }
    }
  ...
  async replyToQuestion(questionId, replyData, userId) {
    try {
      ...
      debug("Found question:", question?.id, "platform:", question?.platform);
      ...
      debug("Reply created:", reply.id);

      // Send reply to platform
      const platformService = this.platformServices[question.platform];
      if (!platformService) {
        debug("Platform service not available for:", question.platform);
      ...
      try {
        ...
        debug("Platform API error:", error.message);

        // Handle "already answered" error specifically
        if (
          error.code === "ALREADY_ANSWERED" ||
          error.message.includes("already answered") ||
          error.message.includes("daha önce cevaplandı") ||
          error.message.includes("business.rule.question.has.already.answered")
        ) {
          debug(
            "Question was already answered on platform, updating local status"
          );

          // Update reply status to indicate it was already answered
          await reply.update({
            status: "failed",
            error_message: "Question was already answered on the platform",
          });

          // Update question status to reflect it was answered
          await question.update({
            status: "ANSWERED",
            answered_date: new Date(),
          });

          // Update template usage if used (since the intent was to use the template)
  ...
  async getReplyTemplates(category = null) {
    try {
      debug("Fetching reply templates, category:", category);
      ...
      debug("Found templates:", templates.length);
      return templates;
    } catch (error) {
      debug("Error in getReplyTemplates:", error.message);
  ...
  async saveReplyTemplate(templateData, userId) {
    try {
      debug("Saving template, data:", templateData, "userId:", userId);
      ...
      if (templateData.id) {
        ...
        debug("Updating existing template:", templateData.id);
      ...
      debug("Error in saveReplyTemplate:", error.message);
      debug("Error stack:", error.stack);
      throw error;
    }
  }
  ...
  /**
   * Sync question status from platform (useful when questions are answered outside the system)
   */
  async syncQuestionStatus(questionId) {
    try {
      const question = await this.getQuestionById(questionId);
      const platformService = this.platformServices[question.platform];

      if (!platformService || !platformService.getQuestionDetails) {
        debug(
          "Platform service does not support status sync for:",
          question.platform
        );
        return question;
      }
      ...
      const platformQuestion = await platformService.getQuestionDetails(
        question.platform_question_id
      );

      if (platformQuestion && platformQuestion.status !== question.status) {
        debug(
          `Updating question ${questionId} status from ${question.status} to ${platformQuestion.status}`
        );

        await question.update({
          status: platformQuestion.status,
          answered_date:
            platformQuestion.status === "ANSWERED"
              ? platformQuestion.answered_date || new Date()
      ...
      debug("Error syncing question status:", error.message);
