...
async function syncVariantCategories() {
    ...
    try {
        ...
        console.log(`   Found ${users.length} users with active platform connections`);
        console.log();
        ...
        for (const user of users) {
            ...
            for (const connection of user.platformConnections) {
                ...
                try {
                    ...
                    if (result.success) {
                        console.log(`   ✅ ${connection.platformType}: ${result.categoriesCount} categories synced`);
                    } else {
        ...
        if (totalSynced > 0) {
            ...
            const mockRes = {
                json: (data) => {
                    console.log(`   ✅ API returned ${data.data.categories.length} categories`);
        ...
        console.log('   The variant tab should now show categories for platform selection.');
