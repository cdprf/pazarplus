...
if (fs.existsSync(serverJsPath)) {
  try {
    ...
    const hasShippingStatic = serverContent.includes('app.use("/shipping"') || 
...
app.use("/shipping", express.static(path.join(__dirname, "public/shipping")));`;
      ...
      if (match) {
        ...
        serverContent = serverContent.replace(match[0], match[0] + staticServingCode);
      } else {
        ...
        for (const pattern of insertPoints) {
          if (serverContent.match(pattern)) {
            serverContent = serverContent.replace(pattern, staticServingCode + '\n\n$&');
            inserted = true;
            break;
          }
        }
        ...
        if (!inserted) {
          ...
          if (serverContent.includes('module.exports')) {
            serverContent = serverContent.replace('module.exports', staticServingCode + '\n\nmodule.exports');
          } else {
...
try {
  ...
  const useAbsoluteUrl = process.env.SERVER_BASE_URL && 
  ...
  // Test URL
  console.log(`🔗 Test this URL: https://yarukai.com${publicUrl.startsWith('/') ? publicUrl : '/' + publicUrl}`);
