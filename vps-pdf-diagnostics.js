...
const testPdfPath = path.join(process.cwd(), 'server/public/shipping/test-pdf-access.pdf');
...
%%EOF`;
  ...
  console.log(`🔗 Test this URL: https://yarukai.com/shipping/test-pdf-access.pdf`);
} catch (err) {
...
if (fs.existsSync(serverJsPath)) {
  try {
    ...
    const staticMatches = serverContent.match(/app\.use\(['"`]\/shipping['"`][^;]*;/g);
    if (staticMatches) {
      ...
      console.log('   You need to add: app.use("/shipping", express.static(path.join(__dirname, "public/shipping")));');
    }
  } catch (err) {
