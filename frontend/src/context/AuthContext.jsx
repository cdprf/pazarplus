...
export const AuthProvider = ({ children }) => {
  ...
  const verifyTwoFactor = useCallback(async (code, secret) => {
    try {
      const response = await axios.post('/auth/2fa/verify', { code, secret });
      ...
      if (token) {
        try {
          ...
          if (currentAuth !== expectedAuth) {
            axios.defaults.headers.common['Authorization'] = expectedAuth;
          }
          ...
          if (err.response?.status === 401 || err.response?.status === 403) {
            ...
            console.warn('Network error during auth verification, keeping session');
          }
        }
      }
      setLoading(false);
