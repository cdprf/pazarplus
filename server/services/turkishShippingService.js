...
class TurkishShippingService {
  ...
  initializeCarriers() {
    ...
    this.carriers.set('YURTICI', {
      ...
      trackingUrl: 'https://www.yurticikargo.com/tr/takip?code={trackingNumber}',
      codSupported: true,
      maxWeight: 50,
    ...
    this.carriers.set('UPS', {
      ...
      trackingUrl: 'https://www.ups.com/track?loc=tr_TR&tracknum={trackingNumber}',
      codSupported: false,
      maxWeight: 70,
      services: ['STANDARD', 'EXPRESS', 'NEXT_DAY']
    });
    ...
    this.carriers.set('MNG', {
      ...
      trackingUrl: 'https://www.mngkargo.com.tr/track?trackingNumber={trackingNumber}',
      codSupported: true,
      maxWeight: 40,
      services: ['STANDARD', 'EXPRESS']
    });
    ...
    try {
      ...
      for (const carrier of activeCarriers) {
        try {
          ...
          logger.warn(`Failed to get rates from ${carrier.name}: ${error.message}`, {
  ...
  async getArasRates(carrier, details) {
    try {
      ...
      const response = await axios.post(`${carrier.apiEndpoint}/api/rate-calculator`, {
        origin: {
  ...
  async getYurticiRates(carrier, details) {
    try {
      ...
      const response = await axios.get(`${carrier.apiEndpoint}/api/price-calculation`, {
        params: {
  ...
  async getPTTRates(carrier, details) {
    try {
      ...
      const response = await axios.post(`${carrier.apiEndpoint}/calculate-shipping`, {
        fromCity: details.fromCity,
        toCity: details.toCity,
        weight: details.weight,
  ...
  async getUPSRates(carrier, details) {
    try {
      ...
      const response = await axios.post(`${carrier.apiEndpoint}/rating`, {
        RateRequest: {
          Shipment: {
            Shipper: {
              Address: {
  ...
  async getMNGRates(carrier, details) {
    try {
      ...
      const response = await axios.post(`${carrier.apiEndpoint}/api/shipping-cost`, {
  ...
  getFallbackRates(carrier, details) {
    ...
    const baseRate = this.calculateBaseRate(details.weight, details.fromCity, details.toCity);
  ...
  calculateBaseRate(weight, fromCity, toCity) {
    ...
    const majorCities = ['İstanbul', 'Ankara', 'İzmir', 'Bursa', 'Antalya', 'Adana'];
  ...
  async createShipment(orderId, carrierId, serviceType, options = {}) {
    try {
      ...
      const shipmentResult = await this.createCarrierShipment(carrier, order, selectedRate, options);
      ...
      if (shipmentResult.success && shipmentResult.trackingNumber) {
        await order.shippingDetail.update({
          ...
          trackingUrl: this.generateTrackingUrl(carrier, shipmentResult.trackingNumber),
          status: 'shipped'
        });
  ...
  generateTrackingUrl(carrier, trackingNumber) {
    ...
    return carrierConfig.trackingUrl.replace('{trackingNumber}', trackingNumber);
  }
