...
class BackgroundTaskService {
  ...
  static async getTaskStats(userId = null, timeframe = "24h") {
    ...
    // Process stats into a more useful format for frontend
    const processed = {
      total: 0,
      byStatus: {},
      byType: {},
    ...
    processed.runningTasks = processed.byStatus.running || 0;
    processed.failedTasks = processed.byStatus.failed || 0;
    processed.totalTasks = processed.total;

    // For completed today, we need to check if timeframe is 24h or less
    if (timeframe === "24h" || timeframe === "1h") {
      processed.completedToday = processed.byStatus.completed || 0;
    } else {
      // For longer timeframes, get completed tasks for today only
      const todayStats = await BackgroundTask.getTaskStats(userId, "24h");
      const todayCompleted = todayStats.find(
        (stat) => stat.status === "completed"
      );
      processed.completedToday = todayCompleted
        ? parseInt(todayCompleted.count)
        : 0;
    }
