...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    console.log('Starting migration: Add errorMessage and retryCount columns to orders table');
    
    // Add errorMessage column if it doesn't exist
    await queryInterface.addColumn('orders', 'errorMessage', {
      type: Sequelize.TEXT,
      allowNull: true,
    ...
    await queryInterface.addColumn('orders', 'retryCount', {
    ...
    await queryInterface.addIndex('orders', ['errorMessage'], {
    ...
    await queryInterface.addIndex('orders', ['retryCount'], {
    ...
    console.log('Finished migration: Added errorMessage and retryCount columns to orders table');
  },

  down: async (queryInterface, Sequelize) => {
    console.log('Starting rollback: Remove errorMessage and retryCount columns from orders table');
    
    // Remove indexes first
    await queryInterface.removeIndex('orders', 'orders_error_message_idx').catch(e => {
      console.log('Error removing index orders_error_message_idx:', e.message);
    });
    await queryInterface.removeIndex('orders', 'orders_retry_count_idx').catch(e => {
    ...
    console.log('Finished rollback: Removed errorMessage and retryCount columns from orders table');
  }
};
