  ...
  async importOrdersFromCSV(file, options = {}) {
    try {
        ...
        if (!orderMap.has(orderId)) {
          orderMap.set(orderId, {
            orderData: {
              ...
              customerEmail: this.getValueFromMapping(
                row,
                columnMap.customerEmail
              ),
              customerPhone: this.getValueFromMapping(
                row,
            ...
            shippingData: {
              recipientName:
                this.getValueFromMapping(row, columnMap.recipientName) ||
                this.getValueFromMapping(row, columnMap.customerName),
              ...
              email:
                this.getValueFromMapping(row, columnMap.email) ||
                this.getValueFromMapping(row, columnMap.customerEmail),
      ...
      for (const [orderId, orderData] of orderMap.entries()) {
        try {
          // Validate and sanitize order data before database operations
          const sanitizedOrderData = { ...orderData.orderData };
          ...
          // Sanitize platform type if present
          if (sanitizedOrderData.platformType) {
            sanitizedOrderData.platformType = sanitizePlatformType(
              sanitizedOrderData.platformType
            );
            if (!sanitizedOrderData.platformType) {
              delete sanitizedOrderData.platformType;
            }
          }

          // Update the orderData object with sanitized data
          orderData.orderData = sanitizedOrderData;
