    ...
    try {
      ...
      lines.forEach((line, index) => {
        ...
        const patterns = [
          {
            regex:
              /import\s+logger\s+from\s+['"]([^'"]*logger)['"](?!\.js|\.jsx|\.ts|\.tsx)/,
            type: "Logger import without .js extension",
            severity: "critical",
          },
          {
            regex:
              /import\s+.*from\s+['"]([^'"]*\/utils\/[^'"]+)['"](?!\.js|\.jsx|\.ts|\.tsx)(?!\/)(?!.*node_modules)/,
            type: "Utils import without .js extension",
            severity: "critical",
          },
          {
            regex:
              /import\s+.*from\s+['"]([^'"]*\/services\/[^'"]+)['"](?!\.js|\.jsx|\.ts|\.tsx)(?!\/)(?!.*node_modules)/,
            type: "Services import without .js extension",
            severity: "warning",
          },
          {
            regex:
              /import\s+.*from\s+['"]([^'"]*\/hooks\/[^'"]+)['"](?!\.js|\.jsx|\.ts|\.tsx)(?!\/)(?!.*node_modules)/,
...
function quickCompilationTest() {
  try {
    ...
    try {
      execSync("npx tsc --noEmit --skipLibCheck 2>/dev/null", {
        stdio: "pipe",
      });
      ...
      try {
        execSync("npm run build --dry-run 2>/dev/null", {
          stdio: "pipe",
          timeout: 10000,
        });
...
function main() {
  ...
  if (issues.length === 0) {
    ...
    if (compileResult === true) {
      ...
      console.log(
        "💡 Consider running full health check: node test-client-health.js"
      );
      process.exit(0);
    }
  }
  ...
  // Show warnings
  if (warnings.length > 0 && critical.length < 10) {
    // Don't overwhelm if too many critical issues
