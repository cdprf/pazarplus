module.exports = {
  up: async (queryInterface, Sequelize) => {
    console.log('Starting migration: Add GIN index to metadata column in orders table');
    ...
    try {
      ...
      if (metadataColumns.length > 0) {
        ...
        if (metadataType === 'json' || metadataType === 'jsonb') {
          console.log(`Creating GIN index on ${metadataType} metadata column...`);
          ...
          console.log(`Metadata column is type ${metadataType}, not JSON or JSONB. Cannot create GIN index.`);
        }
      } else {
    ...
    console.log('Finished migration: Added GIN index to metadata column in orders table');
  },
  
  down: async (queryInterface, Sequelize) => {
    console.log('Starting rollback: Remove GIN index from metadata column in orders table');
    ...
    console.log('Finished rollback: Removed GIN index from metadata column in orders table');
  }
};
