...
const PLATFORM_CONFIG = {
  trendyol: { 
    ...
    color: "#ff6000",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="#ff6000" className="flex-shrink-0">
  ...
  n11: { 
    ...
    color: "#CC0000",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="#CC0000" className="flex-shrink-0">
...
export const productUtils = {
  ...
  getPlatformBadges: (product) => {
    ...
    if (product.platforms) {
      Object.entries(product.platforms).forEach(([platformKey, platformData]) => {
        if (platformData?.enabled && PLATFORM_CONFIG[platformKey] && !activePlatforms.has(platformKey)) {
  ...
  matchesPlatformFilter: (product, platformFilter) => {
    ...
    if (platformFilter === "local") {
      const hasExternalPlatforms = product.sources?.length > 0 || 
  ...
  getLowStockProducts: (products) => {
    return products.filter(product => {
      const stockStatus = productUtils.getStockStatus(product);
      return stockStatus.value === 'low_stock' || stockStatus.value === 'out_of_stock';
    });
  },
...
export const useProductOperations = () => {
  ...
  const setOperationStatus = useCallback((operation, status, productIds = []) => {
    setOperationState(prev => ({
      ...prev,
      [operation]: operation === 'deleting' || operation === 'updating' 
        ? (status ? [...prev[operation], ...productIds] : prev[operation].filter(id => !productIds.includes(id)))
        : status
    }));
  }, []);

  const syncFromPlatforms = useCallback(async (api) => {
    ...
    try {
      ...
      return {
        ...
        message: `Başarıyla ${response.data.data?.totalSaved || 0} ürün senkronize edildi`
      };
    } catch (error) {
      return {
        success: false,
  ...
  const importFromCSV = useCallback(async (api, file) => {
    ...
    try {
      ...
      return {
        success: response.data.success,
        data: response.data.data,
        message: `Başarıyla ${response.data.data?.imported || 0} ürün içe aktarıldı`
      };
    } catch (error) {
      return {
        success: false,
  ...
  const deleteProducts = useCallback(async (api, productIds) => {
  ...
  const updateProduct = useCallback(async (api, productId, productData) => {
...
const UtilitiesDemo = () => {
  ...
  const profitMargin = productUtils.calculateProfitMargin(sampleProduct.price, sampleProduct.costPrice);
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="flex items-center space-x-2">
              {stockDisplay.icon}
              {stockDisplay.badge}
              <span className="text-sm text-gray-600">({stockDisplay.status.label})</span>
            </div>
          </div>

          {/* Platform Badges Demo */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Platform Badges</h3>
            <div className="flex flex-wrap gap-2">
              {platformBadges.map((platform, index) => (
                <Tooltip key={index} content={platform.connectionName || platform.label}>
                  <Badge variant={platform.variant} className="flex items-center gap-1">
                    {platform.icon}
                    {platform.label}
                  </Badge>
                </Tooltip>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Price & Profit</h3>
            <div className="space-y-2">
              <p><span className="text-gray-600">Satış Fiyatı:</span> {productUtils.formatPrice(sampleProduct.price)}</p>
              <p><span className="text-gray-600">Maliyet:</span> {productUtils.formatPrice(sampleProduct.costPrice)}</p>
              <p><span className="text-gray-600">Kar Marjı:</span> %{profitMargin}</p>
            </div>
          </div>

          {/* Validation Demo */}
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Product Validation</h3>
            <div className="space-y-2">
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h3 className="text-lg font-medium text-blue-900 mb-4">✨ Part 2 Features</h3>
