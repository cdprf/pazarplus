...
async function syncAllPlatformCategories() {
    ...
    try {
        ...
        connections.forEach(conn => {
            console.log(`     - ${conn.platformType}: ${conn.name} (User: ${conn.user?.email || conn.userId})`);
        });
        console.log();
        ...
        for (const connection of connections) {
            console.log(`🔄 SYNCING ${connection.platformType.toUpperCase()} CATEGORIES`);
            ...
            try {
                ...
                if (result.success) {
                    console.log(`     ✅ Categories synced: ${result.categoriesSynced || 0}`);
                    console.log(`     🔄 Categories updated: ${result.categoriesUpdated || 0}`);
                    
                    if (result.details) {
                        console.log(`     📋 Details:`);
                        console.log(`       Total processed: ${result.details.totalProcessed || 0}`);
                        console.log(`       New categories: ${result.details.newCategories || 0}`);
                        console.log(`       Updated categories: ${result.details.updatedCategories || 0}`);
        ...
        for (const platform of ['trendyol', 'hepsiburada', 'n11']) {
            ...
            if (count > 0) {
                ...
                sample.forEach(cat => {
                    console.log(`     - ${cat.platformCategoryId}: ${cat.name} (User: ${cat.userId})`);
                });
            }
        }
        console.log();
        ...
        try {
            ...
            console.log(`     Categories count: ${apiResult?.data?.categories?.length || 0}`);
