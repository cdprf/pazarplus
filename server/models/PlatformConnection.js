...
const PlatformConnection = sequelize.define('PlatformConnection', {
  ...
  platformType: {
    ...
    allowNull: false,
    validate: {
      isIn: [['trendyol', 'hepsiburada', 'n11', 'csv']]
    }
  },
  name: {
    type: DataTypes.STRING,
  ...
  status: {
    ...
    defaultValue: 'active',
    validate: {
      isIn: [['active', 'inactive', 'error', 'pending', 'testing', 'suspended']]
    }
  },
  isActive: {
  ...
  indexes: [
    ...
    {
      unique: true,
      fields: ['userId', 'platformType', 'name'],
      name: 'unique_user_platform_name'
    }
  ],
  hooks: {
...
// Class methods
PlatformConnection.getDefaultConnection = async function(userId, platformType) {
...
PlatformConnection.getActiveConnections = async function(userId, platformType = null) {
  const where = {
    userId,
    isActive: true,
    status: 'active'
