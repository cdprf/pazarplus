...
const OrdersChart = () => {
  ...
  const chartData = useMemo(() => {
    ...
    return data.map(item => ({
      ...
      total: (item.newOrders || 0) + (item.processingOrders || 0) + (item.shippedOrders || 0) + (item.deliveredOrders || 0)
    }));
  }, [data]);
    ...
    if (chartType === 'bar') {
      return (
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Legend />
          <Bar dataKey="new" name="New Orders" fill={colors.new} radius={[2, 2, 0, 0]} />
          <Bar dataKey="processing" name="Processing" fill={colors.processing} radius={[2, 2, 0, 0]} />
          <Bar dataKey="shipped" name="Shipped" fill={colors.shipped} radius={[2, 2, 0, 0]} />
          <Bar dataKey="delivered" name="Delivered" fill={colors.delivered} radius={[2, 2, 0, 0]} />
        </BarChart>
      );
    }
      ...
      <div className="chart-container">
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <h4 className="text-lg font-medium text-gray-900 mb-2">Chart Error</h4>
    ...
    <div className="chart-container">
      <div className="chart-header">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Order status distribution over time</p>
        </div>
      ...
      {chartData.length > 0 ? (
        ...
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <h4 className="text-lg font-medium text-gray-900 mb-2">No Chart Data</h4>
            <p className="text-gray-600">No order data available for the selected period</p>
          </div>
        </div>
      )}
    </div>
